...
{
    var html = '';
    html += '<center><img src="https://fortloc.com/quiz/images/splash.jpg" alt="image1" onclick="displayRacers('+ 0 + ')">';
...
function displayRacers(racerNum)
{
    ...
    <center><img src="'+ racers[racerNum].ImageSrc + '" alt="image1"></center>\
    </div>';
   ...
   // html += '<button name=nextButton onclick="displayRacers(' + (racerNum + 1) + ')">Next</button>';
    ...
    <tbody>\
      <tr>\
        ...
        {
            html +=  '<p>'+ racers[racerNum].Name + selection2 + '</p>';
        }
        else{
            html +=  '<p>'+ racers[racerNum].Name + selection1 + '</p>';
        }
        html += '<p><button align="left" name=backButton onclick="displayRacers('+ (racerNum - 1) + ')">Back</button>';
        html += '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;';
        html += '<button name=nextButton onclick="displayRacers('+ (racerNum + 1) + ')">Next</button></p>\
        <td>&nbsp;</td>\
        </tr>\
        </tbody></table></center>';
        html += '<p>'+ racers[racerNum].Description + '</p></td>';   
...
function displayQuiz(no, rightAnswer, fromAnswerSelection, fromTimeOut) {
    ...
    //html += '<div class="quiz-image" width="device-width">';
    if (!rightAnswer)
    {
        html += '<center><img class ="quiz-Anim" src="'+ imageQuestion + '" alt="image1" width="310"></center>';
    }
    else 
    {
        html += '<center><img class ="quiz-Anim" src="'+ contestantImage + '" alt="image1" width="310"></center>'; 
    }
    ...
    if ((fromAnswerSelection && rightAnswer) ||fromTimeOut)
    {
        ...
        html += '<p id="rTime">10</p>';
    }
    if (fromTimeOut || 
        (rightAnswer && fromAnswerSelection) || 
        answered[no] == true)
    {
        html += '<p class="questionP">'+ (currQuestion+1) + '. ' +questions.QuestionsInfo[no].Question + '</p>\
    ...
    html += '</div>\
       <p class="questionP">'+ (currQuestion+1) + '. ' +questions.QuestionsInfo[no].Question + '</p>\
     <form name="myForm" id="answers" class="Select">\
       <ul>\
          <li>\
             <label>\
              <input type="radio"  name="mySelect" onchange="userSelection(\''+ "A" + '\');"   value="a">' + questions.QuestionsInfo[no].OptionA + '\
             </label>\
           </li>\
           <li>\
              <label>\
               <input type="radio" name="mySelect" onchange="userSelection(\''+ "B" + '\')" value="b">' + questions.QuestionsInfo[no].OptionB + '\
              </label>\
          </li>\
          <li>\
              <label>\
                <input type="radio" name="mySelect" onchange="userSelection(\''+ "C" + '\')" value="c">' + questions.QuestionsInfo[no].OptionC + '\
              </label>\
          </li>\
          <li>\
              <label>\
              <input type="radio" name="mySelect" onchange="userSelection(\''+ "D" + '\')" value="d">' + questions.QuestionsInfo[no].OptionD + '\
              </label>\
          </li>\
      </ul>\
     </form>';
...
function displayResult() 
{
    ...
    html += '<button onclick="displayQuiz(' + 0 + ','+false+','+false+'.'+false+')">Play Again</button>';
